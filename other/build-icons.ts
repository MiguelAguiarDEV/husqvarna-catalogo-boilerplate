...
// dir of icons names
const outputIconNamesDir = path.join(cwd, 'src', 'components', 'ui', 'icons');
...
const run = async () => {
  ...
  await fsExtra.ensureDir(outputIconNamesDir);
...
async function generateIconFiles() {
  ...
  const typeOutputFilepath = path.join(outputIconNamesDir, 'name.d.ts');
...
The names of the icons for its use are in the dir: ${path.relative(cwd, outputIconNamesDir)}
...
`
  ...
  await writeIfChanged(
    path.join(outputIconNamesDir, 'README.md'),
    `# Icons

This directory contains the names of the SVG icon used by the app.

SVG icons that are used by the app are in this directory: ${path.relative(cwd, outputDir)}
